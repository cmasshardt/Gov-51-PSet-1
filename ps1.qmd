...
```{r}
#Create variable no_commute with value of 1 if the person has no
#commute (commute time of 0 minutes) and 0 otherwise 
#(commute time greater than 0 minutes).
acs <- acs %>%
...
#Since 1 represents no commute and 0 represents a positive commute
#time, the mean of no_commute gives the proportion of people with 
#a commute time of 0 minutes (represented by a value of 1).
mean(acs$no_commute)
```
...
commuter_stats <- data.frame(
  ...
  N = c(length(commuters$AGE),
        ...
        length(commuters$less_than_hs),    length(commuters$hs_only),
...
```{r}
ggsave("./output/commute_histogram.png", commute_histogram,
       width = 8, height = 5)
```

## 5. The Weight of Evidence

*1. Calculate the weighted mean (commute time).*

```{r}
#Weighted mean using PERWT variable
...
Yes: The weighted mean commute time of 27.19 minutes is slightly lower than the unweighted mean of 27.22 minutes, as calculated earlier. This is a rather insignificant difference of 0.03 minutes.

*3. In 2-3 sentences, explain what each number represents and why they might differ.*

The unweighted mean of 27.22 minutes represents the mean commute time for an individual in the ACS sample who is a commuter; meanwhile, the *weighted* mean of 27.19 minutes represents the mean commute time for an individual in the general population who commutes to work. The weighted mean takes into account how many people in the overall American population are represented by each particular survey respondent with regard to other characteristics such as age and income, such that the weighted mean better represents the commute time for the "average" American. As such, the weighted mean commute time might be slightly lower than the unweighted mean because it could, for example, better represent younger people who walk/bike to work, or who work closer to home so that they can be in close proximity to their children. 
